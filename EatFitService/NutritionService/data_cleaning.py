...
def fill_product_names_and_images():
    ...
    products = Product.objects.filter(Q(quality_checked__isnull=True) | Q(quality_checked__lt=one_month_before),
                                      product_name_de__isnull=True, data_score__lt=20.0,
                                      automatic_update=True).order_by("quality_checked")[:50]
    for product in products:
        update_from_openfood(product,
                             {"product_name_de": 1, "product_name_en": 1, "product_name_fr": 1, "product_name_it": 1,
                              "image": 1, "product_size": 1, "product_size_unit_of_measure": 1, "portion_quantity": 1,
                              "nutrients": 1})
