...
def test_basket_analysis_api_matching_non_found_exists():
    ...
    expected_results = models.NonFoundMatching.objects.get(article_id=ARTICLE_ID_SIMPLE,
                                                           article_type=ARTICLE_TYPE_SIMPLE)

    assert expected_results.counter == 1
...
def test_basket_analysis_api_matching_multiple():
    ...
    expected_results = models.Matching.objects.filter().first()

    assert expected_results.gtin == 0


@pytest.mark.django_db
def test_basket_analysis_api_matching_multiple_price_does_not_exist():
    assert User.objects.count() == 0
    assert models.ReceiptToNutritionPartner.objects.count() == 0
    assert models.ReceiptToNutritionUser.objects.count() == 0
    assert models.Matching.objects.count() == 0
    ...
    expected_results = models.Matching.objects.filter(article_id=ARTICLE_ID_SIMPLE,
                                                      article_type=ARTICLE_TYPE_SIMPLE,
...
@pytest.mark.django_db
def test_basket_analysis_api_matching_multiple_some_prices_exist():
    assert User.objects.count() == 0
    assert models.ReceiptToNutritionPartner.objects.count() == 0
    assert models.ReceiptToNutritionUser.objects.count() == 0
    assert models.Matching.objects.count() == 0
    ...
    assert expected_results != models.Matching.objects.filter().first()
    assert expected_results.gtin == 1


@pytest.mark.django_db
def test_basket_analysis_api_matching_multiple_by_price():
    assert User.objects.count() == 0
    assert models.ReceiptToNutritionPartner.objects.count() == 0
    assert models.ReceiptToNutritionUser.objects.count() == 0
    assert models.Matching.objects.count() == 0
    ...
    expected_results = models.Matching.objects.filter(article_id=ARTICLE_ID_SIMPLE,
                                                      article_type=ARTICLE_TYPE_SIMPLE).annotate(
        absolute_price_difference=Func(F('price_per_unit') - PRICE, function='ABS')).order_by(
        'absolute_price_difference').first()
